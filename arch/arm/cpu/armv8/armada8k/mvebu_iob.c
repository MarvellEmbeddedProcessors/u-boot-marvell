...
void dump_iob(void)
{
	u32 win_id, win_cr, alr, ahr, max_win;
	u8 target_id;
	u64 start, end;
	const void *blob = gd->fdt_blob;
	void __iomem *iob_base;
	int node, node_list[IOB_MAX_UNIT], count, i;
	char *iob_target_name[IOB_MAX_TID] = {"CONFIG", "IHB0  ", "PEX1  ", "PEX2  ",
					      "PEX0  ", "NAND  ", "RUNIT ", "IHB1  "};

	count = fdtdec_find_aliases_for_id(blob, "iob",
			COMPAT_MVEBU_IOB, node_list, IOB_MAX_UNIT);
