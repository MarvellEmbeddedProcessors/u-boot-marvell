...
void misc_init_r_env(void)
{
	...
	if (!env)
		setenv("bootcmd_auto", "setenv bootargs_end :$gatewayip:255.255.255.0:Armada:$netdev:none; "
				"stage_boot $boot_order");
	env = getenv("bootcmd_lgcy");
	if (!env)
		setenv("bootcmd_lgcy", "setenv bootargs_end :$gatewayip:255.255.255.0:Armada:$netdev:none; "
			"tftpboot 0x2000000 $image_name;"
	...
	env = getenv("bootcmd");
	if (!env)
		setenv("bootcmd", "setenv bootargs_end :$gatewayip:255.255.255.0:Armada:$netdev:none; "
