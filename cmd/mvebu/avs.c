...
#define AVS_ENABLE_MASK		(1 << 0)
#define AVS_SOFT_RESET_MASK	(1 << 2)
#define AVS_CFG_DEBUG_SELECT	(0x1 << 29)
	       ...
	       char * const argv[])
{
	if (strncmp(argv[1], "get", 3) == 0) {
		...
		if ((val_mv < 700) || (val_mv > 1100)) {
			printf("Requested value %ldmV is out of the supported range\n",
			       val_mv);
			return 0;
		}
		do_set_avs(val_mv);
	} else {
		printf("Bad format. Use \"help avs\" for more info about this command\n");
	}
