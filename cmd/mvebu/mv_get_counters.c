...
#define MVEBU_REGS_BASE_CP(cp)		(0xf2000000ULL + (cp) * 0x02000000)
#define COUNTERS_BASE			0x129000
#define GOP_OFFSET			(((gop) + ((gop) != 0)) * 0x100)
#define COUNTERS_ADDRESS(cp, gop_p)	(COUNTERS_BASE + \
					MVEBU_REGS_BASE_CP(cp) + GOP_OFFSET)
...
int mv_do_get_counters_cmd(struct cmd_tbl *cmdtp, int flag,
...
# if 0 /* TODO: restore this code once SOC services are added */
		u32 cp_max;
		u32 ap_max;
