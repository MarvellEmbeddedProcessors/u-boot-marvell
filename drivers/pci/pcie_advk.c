...
/**
 * pcie_advk_check_pio_status() - Validate PIO status
 *
 * @pcie: Pointer to the PCI bus
 *
 */
static void pcie_advk_check_pio_status(struct pcie_advk *pcie)
{
	uint reg;
	unsigned int status;
	...
	error("%s PIO Response Status: %s, %#x @ %#x\n",
				 ...
				 enum pci_size_t size)
{
	...
	pcie_advk_check_pio_status(pcie);

	/* Get the read result */
	reg = advk_readl(pcie, PIO_RD_DATA);
...
static int pcie_advk_ofdata_to_platdata(struct udevice *dev)
{
	...
	pcie->base = (void *)dev_get_addr_index(dev, 0);
