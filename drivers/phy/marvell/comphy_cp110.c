...
struct comphy_mux_data cp110_comphy_phy_mux_data[] = {
	...
	/* Lane 4 */
	{7, {{COMPHY_TYPE_UNCONNECTED, 0x0}, {COMPHY_TYPE_SGMII1, 0x2},
	     {COMPHY_TYPE_RXAUI0, 0x2}, {COMPHY_TYPE_SFI, 0x2},
	     {COMPHY_TYPE_SGMII2, 0x1} } },
	/* Lane 5 */
				 ...
				 u32 lane)
{
	...
	if (ptr_chip_cfg->comphy_map_data[lane].type != COMPHY_TYPE_SFI) {
		error("Comphy %d isn't configured to SFI\n", lane);
		return 0;
	}
...
void comphy_dedicated_phys_init(void)
{
	...
	for (node_idx = 0; node_idx < MAX_UTMI_PHY_COUNT;) {
		...
		if (cp110_utmi_data[node_idx].utmi_base_addr == NULL) {
		...
		if (cp110_utmi_data[node_idx].usb_cfg_addr == NULL) {
			error("UTMI PHY base address is invalid\n");
			continue;
		}
		...
		if (cp110_utmi_data[node_idx].utmi_cfg_addr == NULL) {
			error("UTMI PHY base address is invalid\n");
			continue;
		}
