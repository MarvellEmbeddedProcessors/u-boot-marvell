...
static struct thermal_unit_config *init_thermal_config(struct udevice *dev)
{
	...
	/* Read register base */
	thermal_cfg->regs_base = (void *)dev_get_addr_index(dev, 0);

	if (thermal_cfg->regs_base == 0) {
		error("thermal: base address is missing in device-tree\n");
		return NULL;
	}

	/* Register functions (according to compatible) */
	if (of_device_is_compatible(dev, "marvell,thermal-sensor")) {
	...
	if (of_device_is_compatible(dev, "marvell,thermal-ext-sensor")) {
		thermal_cfg->ptr_thermal_sensor_probe =
	...
	if (!thermal_cfg->ptr_thermal_sensor_probe ||
	    !thermal_cfg->ptr_thermal_sensor_read) {
		error("thermal.%lx: compatible is not supported\n",
...
static int mvebu_thermal_probe(struct udevice *dev)
{
	...
	if (thermal_cfg == NULL) {
		error("Thermal: failed to initialize thermal data structure\n");
		return -1;
	}
